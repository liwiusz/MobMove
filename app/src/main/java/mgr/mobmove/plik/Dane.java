package mgr.mobmove.plik;

/**
 * Created by mliwi on 01.09.2016.
 */
public class Dane {

    public String czas;
    public String Xa;
    public String Ya;
    public String Za;
    public String orientationX;
    public String orientationY;
    public String orientationZ;

    public String gravityX;
    public String gravityY;
    public String gravityZ;

    public String gyroscopeX;
    public String gyroscopeY;
    public String gyroscopeZ;

    public String gyroscopeUXa;
    public String gyroscopeUYa;
    public String gyroscopeUZa;
    public String gyroscopeUXb;
    public String gyroscopeUYb;
    public String gyroscopeUZb;

    public String linearAccelometerX;
    public String linearAccelometerY;
    public String linearAccelometerZ;

    public String rotationX;
    public String rotationY;
    public String rotationZ;

    public String krok;
    public String xMap;
    public String yMap;

    public String magneticX;

    public String getMagneticY() {
        return magneticY+";";
    }

    public void setMagneticY(String magneticY) {
        this.magneticY = magneticY;
    }

    public String getMagneticZ() {
        return magneticZ+";";
    }

    public void setMagneticZ(String magneticZ) {
        this.magneticZ = magneticZ;
    }

    public String getMagneticX() {
        return magneticX+";";
    }

    public void setMagneticX(String magneticX) {
        this.magneticX = magneticX;
    }

    public String magneticY;
    public String magneticZ;

    public String getDystans() {
        return dystans+";";
    }

    public void setDystans(String dystans) {
        this.dystans = dystans;
    }

    public String dystans;

    public String getKrok() {
        return krok+";";
    }

    public void setKrok(String krok) {
        this.krok = krok;
    }

    public String getyMap() {
        return yMap+";";
    }

    public void setyMap(String yMap) {
        this.yMap = yMap;
    }

    public String getxMap() {
        return xMap+";";
    }

    public void setxMap(String xMap) {
        this.xMap = xMap;
    }

    public String getLinearAccelometerX() {
        return linearAccelometerX+";";
    }

    public void setLinearAccelometerX(String linearAccelometerX) {
        this.linearAccelometerX = linearAccelometerX;
    }

    public String getOrientationX() {
        return orientationX+";";
    }

    public void setOrientationX(String orientationX) {
        this.orientationX = orientationX;
    }

    public String getOrientationY() {
        return orientationY+";";
    }

    public void setOrientationY(String orientationY) {
        this.orientationY = orientationY;
    }

    public String getOrientationZ() {
        return orientationZ+";";
    }

    public void setOrientationZ(String orientationZ) {
        this.orientationZ = orientationZ;
    }

    public String getGravityX() {
        return gravityX+";";
    }

    public void setGravityX(String gravityX) {
        this.gravityX = gravityX;
    }

    public String getGravityY() {
        return gravityY+";";
    }

    public void setGravityY(String gravityY) {
        this.gravityY = gravityY;
    }

    public String getGravityZ() {
        return gravityZ+";";
    }

    public void setGravityZ(String gravityZ) {
        this.gravityZ = gravityZ;
    }

    public String getGyroscopeX() {
        return gyroscopeX+";";
    }

    public void setGyroscopeX(String gyroscopeX) {
        this.gyroscopeX = gyroscopeX;
    }

    public String getGyroscopeY() {
        return gyroscopeY+";";
    }

    public void setGyroscopeY(String gyroscopeY) {
        this.gyroscopeY = gyroscopeY;
    }

    public String getGyroscopeZ() {
        return gyroscopeZ+";";
    }

    public void setGyroscopeZ(String gyroscopeZ) {
        this.gyroscopeZ = gyroscopeZ;
    }

    public String getGyroscopeUXa() {
        return gyroscopeUXa+";";
    }

    public void setGyroscopeUXa(String gyroscopeUXa) {
        this.gyroscopeUXa = gyroscopeUXa;
    }

    public String getGyroscopeUYa() {
        return gyroscopeUYa+";";
    }

    public void setGyroscopeUYa(String gyroscopeUYa) {
        this.gyroscopeUYa = gyroscopeUYa;
    }

    public String getGyroscopeUZa() {
        return gyroscopeUZa+";";
    }

    public void setGyroscopeUZa(String gyroscopeUZa) {
        this.gyroscopeUZa = gyroscopeUZa;
    }

    public String getGyroscopeUXb() {
        return gyroscopeUXb+";";
    }

    public void setGyroscopeUXb(String gyroscopeUXb) {
        this.gyroscopeUXb = gyroscopeUXb;
    }

    public String getGyroscopeUYb() {
        return gyroscopeUYb+";";
    }

    public void setGyroscopeUYb(String gyroscopeUYb) {
        this.gyroscopeUYb = gyroscopeUYb;
    }

    public String getGyroscopeUZb() {
        return gyroscopeUZb+";";
    }

    public void setGyroscopeUZb(String gyroscopeUZb) {
        this.gyroscopeUZb = gyroscopeUZb;
    }

    public String getLinearAccelometerY() {
        return linearAccelometerY+";";
    }

    public void setLinearAccelometerY(String linearAccelometerY) {
        this.linearAccelometerY = linearAccelometerY;
    }

    public String getLinearAccelometerZ() {
        return linearAccelometerZ+";";
    }

    public void setLinearAccelometerZ(String linearAccelometerZ) {
        this.linearAccelometerZ = linearAccelometerZ;
    }

    public String getRotationX() {
        return rotationX+";";
    }

    public void setRotationX(String rotationX) {
        this.rotationX = rotationX;
    }

    public String getRotationY() {
        return rotationY+";";
    }

    public void setRotationY(String rotationY) {
        this.rotationY = rotationY;
    }

    public String getRotationZ() {
        return rotationZ+";";
    }

    public void setRotationZ(String rotationZ) {
        this.rotationZ = rotationZ;
    }





    public String getSpeed() {
        return speed+";";
    }

    public void setSpeed(String speed) {
        this.speed = speed;
    }

    public String speed;


    public String getZa() {
        return Za;
    }

    public void setZa(String za) {
        Za = za+";";
    }

    public String getXa() {
        return Xa;
    }

    public void setXa(String xa) {
        Xa = xa+";";
    }

    public String getCzas() {
        return czas;
    }

    public void setCzas(String czas) {
        this.czas = czas+";";
    }



    public String getYa() {
        return Ya;
    }

    public void setYa(String ya) {
        Ya = ya+";";
    }



}
